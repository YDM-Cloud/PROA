function statistics_result=statistics_data(D)
addpath("algorithms\");
addpath("CEC\");
...
statistics_result=cell(n_cec*2+1,n_alg+1);
...
for i=1:n_cec*2
    b=mod(i,2);
    a=ceil(i/2);
    row_name=metaclass(CEC_functions{a}).Name;
    if b==1
        row_name=[row_name,'_mean'];
    else
        row_name=[row_name,'_std'];
    end
...
for i=1:n_cec
    ...
    for j=1:n_alg
        ...
        p_mean=mean(p_bests);
        p_std=std(p_bests);
        statistics_result{2*i,j+1}=p_mean;
        statistics_result{2*i+1,j+1}=p_std;
    end
end
