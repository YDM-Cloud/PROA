function statistics_result=statistics_p_history(D)
addpath("algorithms\");
addpath("CEC\");
...
for i=1:n_cec
    ...
    for j=1:n_alg
        ...
        sql=sprintf('select id from base_info where name="%s" and F="%s" and D=%d order by idx',alg_name,f_name,sql_D);
        ids=fetch(conn,sql);
        ids=cell2mat(ids);
        n_ids=length(ids);
        ids_str=mat2str(ids);
        ...
        ids_str(1)='(';
        ids_str(end)=')';
        sql=sprintf('select iter_best from iter_history where base_info_id in %s order by base_info_id,iter_idx',ids_str);
        histories=fetch(conn,sql);
        histories=cell2mat(histories);
        histories=reshape(histories,500,n_ids)';
        statistics_result{i+1,j+1}=mean(histories);
    end
end
